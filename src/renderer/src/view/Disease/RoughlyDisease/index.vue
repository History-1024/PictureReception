  ...
  <div>
    ...
    <div v-loading="loading" element-loading-text="正在请求数据中，请稍后……" class="bottom">
      <el-tabs v-model="activeName" type="border-card" class="demo-tabs">
        ...
        <el-tab-pane label="薄弱地段明细" name="two">
          <el-table
            :data="weaknessData"
            max-height="calc(100vh - 150px)"
            style="width: 100%"
...
const getTableData = async () => {
  try {
    ...
    if (res.code === '200') {
      ...
      const sortData = res.data.repeatInfoList.sort((p1, p2) => {
        return p2.repeatCount - p1.repeatCount
      })
      weaknessData.value = sortData.slice(0, 10)
